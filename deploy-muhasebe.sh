#!/bin/bash
# Muhasebe (İnşaat/Stok Yönetimi) — tek komutluk deploy
# Kullanım:  ./deploy-muhasebe.sh "commit notu"
#   1) frontend build   2) commit + push   3) build rsync
#   4) sunucuda deploy   5) sağlık kontrolü
set -euo pipefail

SERVER=root@49.13.68.63
APP_REMOTE=/home/boztepeler.com/muhasebe
URL=https://muhasebe.boztepeler.com
...
ssh -o BatchMode=yes "$SERVER" '/root/deploy-muhasebe.sh'
